...
log = core.getLogger()
rule1 = [EthAddr("00:00:00:00:00:01"),EthAddr("00:00:00:00:00:04")]
rule2 = [EthAddr("00:00:00:00:00:02"),EthAddr("00:00:00:00:00:03")]
rule3 = []
...
class VideoSlice (EventMixin):
    ...
    # Handle links between two ports.
    ...
    def _handle_PacketIn (self, event):
		...
		#Sending packets source to destination
        def forward (message = None):
				this_dpid = dpid_to_str(event.dpid);
				if (packet.src in rule1 and packet.dst in rule1) or (packet.src in rule2 and packet.dst in rule2) or (packet.src in rule3 and packet.dst in rule3):
								log.debug("There is a firewall rule blocking a packet")
				else:
					if packet.dst.is_multicast:
						...
						try:
							...
							if self.same_side.get(traffic) is not None:
								...
								if tcpp.srcport == 10000 or tcpp.dstport == 10000:
